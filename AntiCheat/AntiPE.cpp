...
bool IsFondHackDll()
{
	if (s_bHackDll)
	{
		...
		PrintDbgInfo(_T("公司%s || dll：%s, 需要上传到服务器"), fileLeftV.GetCompanyName().c_str(), g_szLeftDll);
		PrintDbgInfo(_T("公司%s || dll：%s, 需要上传到服务器"), fileRightV.GetCompanyName().c_str(), g_szRightDll);
	}
	return true;
}
